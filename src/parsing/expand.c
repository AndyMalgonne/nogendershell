...
/*   Updated: 2025/02/21 00:06:44 by gmoulin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static char	*replace_exit_status(char *token_value, int exit_status, size_t i)
{
	char	*status_str;
	char	*tmp;
	char	*result;
...
int	expand(t_token *tokens, t_env *env, int exst)
{
	while (tokens)
	{
		...
		{
			if (!expand_token_value(tokens, env, exst))
				return (0);
		}
		tokens = tokens->next;
	}
