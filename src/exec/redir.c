...
/*   Updated: 2025/02/21 12:35:20 by amalgonn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	handle_fd_error(t_iofile *io, t_fds *fds)
{
...
void	redir(t_fds *fds, int pip[2], t_tree *cmd, t_var *var)
{
	if (fds->prev_fd != -1)
	{
		if (dup2(fds->prev_fd, STDIN_FILENO) == -1)
			(mclose(&(fds->prev_fd)), exit(1));
		mclose(&(fds->prev_fd));
	}
	if (fds->infd > 0)
	{
		if (dup2(fds->infd, STDIN_FILENO) == -1)
			(mclose(&(fds->infd)), free_all(cmd, var), exit(1));
		mclose(&(fds->infd));
	}
	if (cmd->next)
	{
	...
	if (fds->outfd > 1)
	{
		if (dup2(fds->outfd, STDOUT_FILENO) == -1)
			(mclose(&(fds->outfd)), free_all(cmd, var), exit(1));
		mclose(&(fds->outfd));
	}
	mclose(&pip[0]);
	mclose(&pip[1]);
